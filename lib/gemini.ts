...
### 1. 핵심 요약 (최상단)
- 보고서 제목(h1) 아래에 가장 먼저 배치
- "${keyword}" 관련 최근 7일 흐름을 3~5문장으로 압축 요약
- 전체 동향과 가장 주목할 점을 한눈에 파악할 수 있게 작성

### 2. 주요 이슈 TOP 5
- 수집된 기사를 분석해 가장 중요한 이슈 5개를 선정
- 이슈가 5개 미만이면 있는 만큼만 작성
- 각 이슈는 h2로 번호와 제목을 표기 (예: "1. ○○○")

### 3. 이슈별 상세 (각 TOP 5 항목마다 아래 3가지를 반드시 포함)
각 이슈마다 h3 소제목 3개를 사용하여 구분:
- **핵심 내용**: 해당 이슈의 사실 관계와 주요 내용을 2~4문장으로 설명
- **중요한 이유**: 이 이슈가 "${keyword}" 분야에서 왜 주목받는지, 업계·사회적 영향을 2~3문장으로 설명
- **출처**: 관련 기사 제목과 링크를 ul/li 목록으로 제시 (수집된 기사 링크 활용)

## 작성 규칙
- 한국어로 작성
- 객관적이고 전문적인 톤 유지
- HTML 형식으로 작성 (h1, h2, h3, p, ul, li, a 태그 사용)
- 각 섹션을 명확히 구분
